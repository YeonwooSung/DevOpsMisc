...
def run_sample():
    ...
    for text in texts:
        detect_intent_texts_test(project_id, location_id, agent_id, session_id, text, language_code)
...
def detect_intent_texts_test(project_id, location_id, agent_id, session_id, text, language_code):
    """Returns the result of detect intent with texts as inputs.
    Using the same `session_id` between requests allows continuation
    of the conversation."""
    agent = f"projects/{project_id}/locations/{location_id}/agents/{agent_id}"
